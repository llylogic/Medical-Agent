...
- **技术实践**：突破纯检索局限，封装了读/写/逻辑验证的 5 大异构工具。针对复杂环境设计了两大容错机制：
  - **SQL 异常反射 (Self-Correction)**：Text2SQL 语法报错时，系统截获 Traceback 传回图节点，触发大模型“自主反思并重写 SQL”，使跨表查询成功率提升超 40%。
  - **工具观测值压缩 (Observation Compression)**：当外部联网工具（Web Search）抓取到超长网页源数据时，利用旁路轻量级子模型前置介入，将其提炼为高密度短文本后再注入 ReAct 上下文，彻底杜绝了外部脏数据导致的主模型 Token 溢出与系统崩溃。

**3. 【记忆管理层】设计三级立体记忆架构解决“失忆与混淆”痛点**
- **技术实践**：摒弃了易引发语义损耗的外挂 Query Rewrite，重构了智能体的原生记忆流转体系：
  - **瞬时记忆 (System Injection)**：捕获前端登录态，将 UserID 隐式注入底层的 System Prompt。实现“零感知”身份锚定，大模型在生成 SQL 查表时可全自动使用主键溯源。
  - **工作记忆 (Working Memory)**：利用 LangGraph 的 `Append-only Message Tree` 维持短时多轮 ReAct 推理状态。
  - **滚动压缩记忆 (Context Compression)**：当对话轮次触及安全阈值，异步拉起子模型对远古对话进行高密度实体提炼，替换冗长历史，彻底消除了深层循环中的“注意力稀释 (Lost in the middle)”。

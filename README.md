...
- **核心能力**：基于 LangGraph 白盒引擎，横向挂载了结构化病历查询（Text2SQL）、RAG 知识库、风控预警、PubMed 国际医学期刊（Skill 插件）及挂号写入等 5 大工具。系统具备全链路动作可观测、工具结果动态压缩、SQL 异常自我纠错，以及三层立体的智能体记忆架构。
...
- **技术实践**：突破纯检索局限，封装了读/写/逻辑验证的 5 大异构工具。针对复杂环境设计了两大容错机制：
  ...
  - **工具观测值压缩**：当外部检索工具（international_medical_literature_skill）抓取到超长网页源数据时，利用旁路轻量级子模型前置介入，将其提炼为高密度短文本后再注入 ReAct 上下文，总结文章内容的同时还防止了外部脏数据导致的主模型 Token 溢出与系统崩溃。
...
**4. 【安全与交互层】企业级鉴权状态隔离与动态意图路由**
- **技术实践**：
  - 将会话鉴权（Accounts 表）与业务数据（Patients 表）基于 3NF 范式解耦。在前端实现硬编码的 `Logout` 清理流，一键复位所有全局状态与监听日志，保障终端数据的绝对安全隔离。
  - **风控解耦**：采用 Adapter（适配器）模式，将医疗配伍禁忌规则抽离为独立的 Knowledge Graph API 插件（Skill），彻底脱离底层 Prompt 硬编码。
  - 重构意图路由网络，对日常闲聊实施“懒加载”直接回复，杜绝工具滥用；仅在触发核心医疗决策时才拉起全链路深度校验。
